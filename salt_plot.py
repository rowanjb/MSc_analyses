...
def salinity_plots(variable, mask, time_slice): 
    for i in [1,2]:
        for depth in ['2000']:#['50','200','1000','2000']:
            ...
            if variable=='FWC': ax1.set_ylabel('FWC ($m^3$)')
            if variable=='vosaline': ax2.set_ylabel('Salinity ($PSU$)') 
            #ax1.set_xlabel('Time')
            ...
            ax2.set_ylim(bottom=min(avg_values)-300000000000, top=max(avg_values)+100000000000)
...
if __name__ == "__main__":
    for variable in ['FWC']:#['vosaline','FWC']:
        for mask in ['LS3k']:#['LS2k','LS','LSCR']:
            for time_slice in [False]:#[True, False]:
