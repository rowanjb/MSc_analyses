...
def convR(run, mask_choice, depth, movie=False):
    ...
    g = 9.80665 #gravity
    DS = DS.where(DS.deptht < depth, drop=True) #drop values below specified depth
    ...
    bottom_slice = DS.votemper.isel(deptht = -1).isel(time_counter = 0)
    bottom_slice_bool = bottom_slice.notnull()
    shelf_mask, temp = xr.broadcast(bottom_slice_bool, DS.votemper.isel(time_counter=0))
    DS = DS.where(shelf_mask)

    #pre-calculations
    areas = DS.e1t*DS.e2t
    areas = areas.isel(deptht=0) #only need one slice
    area = areas.sum(dim=['x_grid_T','y_grid_T'])
    Th = DS.votemper.isel(deptht=-1) #temperature values in the deepest level
    Sh = DS.vosaline.isel(deptht=-1) #salinity values in the deepest level
    refDens = density.density(Sh,Th) #potential densities at (around) h
    dens = density.density(DS.vosaline,DS.votemper) #densities in each cell from surface to h

    #following the equation from "Introducing LAB60..."
    term1 = refDens*(DS.e3t.sum(dim='deptht')) #used e3t.sum instead of depth[-1] because term2 is integrated using e3t too
    term2 = dens*DS.e3t
    ...
    convR_timePlot = convR_col*areas*DS.deptht.isel(deptht=-1) #multiply the J/m3 value by column area and depth to get the total J  #/area #multiplying by cell area and dividing by total area (i.e., essentially weighting the values)
    ...
    #saving
    convR_col.to_netcdf(run + '_convR/' + run + '_convR_map_TEST_noloop_' + mask_choice + str(depth) + '.nc')
    convR_timePlot.to_netcdf(run + '_convR/' + run + '_sumConvR_plot_TEST_noloop_' + mask_choice + str(depth) + '.nc') 
...
if __name__ == '__main__':
    for d in [50,200,1000,2000]:
        convR(run='EPM158',mask_choice='LS',depth=d,movie=False) #NOTE: looping through the four depths within the function quadruples the time but uses the same memory
