...
def MLE_plot(mask, time_slice=False):
    ...
    for run in ['EPM151','EPM152','EPM155','EPM156','EPM157','EPM158']:
        path = run + '_MLE/' + run + '_MLE_Q_timeplot_sum_rolling_' + mask + '.nc' #run + '_MLE/' + run + '_MLE_Q_timeplot_' + mask + '.nc' 
    ...
    titl = 'Total heat flux from MLEs across the ML in \nthe Labrador Sea ' + mask_description
    plt.title(titl)
    plt.ylabel('$Q$ ($W$) ???') 
    plt.xlabel('Time')
    name = 'pics_MLE/MLE_Q_rolling_timeplot_' + mask + '_over_time.png'
    if time_slice: name = 'pics_MLE/MLE_Q_rolling_timeplot_' + mask + '_over_time_2010-2014.png' 
    plt.savefig(name, dpi=900, bbox_inches="tight")
    plt.clf()

if __name__ == "__main__":
    for mask in ['LSCR','LS']: #'LS2k'
