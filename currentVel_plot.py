...
def currentVel_plot(mask, d, time_slice=False):
    ...
    for run in ['EPM157','EPM151','EPM155','EPM158','EPM152','EPM156']:
        path = run + '_currentVel/' + run + '_avgCurrentVelMag_timeplot_' + mask + d + '.nc' #run + '_bling/' + run + '_' + tracer + '_timeplot_' + mask + d + '.nc' 
        data2plot = xr.open_dataarray(path)
        data2plot = data2plot.rolling(time_counter=19,center=True).mean()
    ...
    titl = 'Average current speed in the\nLabrador Sea ' + mask_description
    plt.title(titl)
    plt.ylabel('Speed ($m$ $s^{-1}$)')
    plt.xlabel('Time')
    name = 'pics_currentVel/avgCurrentVelMag_timeplot_' + mask + d + '_over_time.png'
    if time_slice: name = 'pics_currentVel/avgCurrentVelMag_timeplot_' + mask + d + '_over_time_2010-2014.png'
    plt.savefig(name, dpi=900, bbox_inches="tight")
    plt.close(fig)


if __name__ == "__main__":
    for mask in ['LS2k']:#,'LSCR','LS2k']:
        for depth in ['1000','1000','2000','200','50']:
            currentVel_plot(mask,depth)
