...
def currentVelocity(run,mask_choice,mapORtimeplot):
    ...
    for d in [50, 200, 1000, 2000]: #loop through depths
        ...
        DS_d['Uz'] = DS_d.U.mean(dim=['z'])#,'time_counter'])
        DS_d['Vz'] = DS_d.V.mean(dim=['z'])#,'time_counter'])
        
        if mapORtimeplot=='map': #I've changed this since last use, but it should be the same
            DS_d['Uz'] = DS_d.Uz.mean(dim=['time_counter'])
            DS_d['Vz'] = DS_d.Vz.mean(dim=['time_counter'])
            DS_d = DS_d.drop_vars(['vosaline','z','U','V','time_counter'])
            DS_d.to_netcdf(run + '_currentVel/' + run + '_avgCurrentVel_map_' + mask_choice + str(d) + '.nc')
        elif mapORtimeplot=='timeplot':
            DS_d['vel'] = (DS_d.Uz**2 + DS_d.Vz**2)**0.5 #this will be the magnitude of the velocity, ie always positive
            DS_d['avg_vel_mag'] = DS_d.vel.mean(dim=['x_grid_T','y_grid_T'])
            DS_d.avg_vel_mag.to_netcdf(run + '_currentVel/' + run + '_avgCurrentVelMag_timeplot_' + mask_choice + str(d) + '.nc')
...
if __name__ == '__main__': #one mask, all 6 runs, timeplot, took <36 hours and <10gb 
    mask_choice = 'LSCR' #choose which mask; options are 'LSCR', 'LS2k', or 'LS'
    for run in ['EPM151','EPM152','EPM155','EPM156','EPM157','EPM158']:
        currentVelocity(run,mask_choice,mapORtimeplot='timeplot')
